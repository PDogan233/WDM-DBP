...
namespace ZK
{
    ...
    */
    ...
    public:
         ...
         */
        Mat(Num_T* const c_array, int rows, int cols, bool row_major = true,
        ...
        */
        Mat<Num_T> operator()(size_t r1, size_t r2, size_t c1, size_t c2) const;
        /*!
          \brief Sub-matrix from row \c r1 to row \c r2 and columns \c c1 to \c c2.

          Value -1 indicates the last row and column, respectively.
        */
        Mat<Num_T> get(size_t r1, size_t r2, size_t c1, size_t c2) const;

        //! Get row \c r
        Vec<Num_T> get_row(size_t r) const;
        //! Get rows \c r1 through \c r2
        Mat<Num_T> get_rows(size_t r1, size_t r2) const;
        //! Get the rows specified by \c indexlist
        Mat<Num_T> get_rows(const Vec<size_t>& indexlist) const;
        //! Get column \c c
        Vec<Num_T> get_col(size_t c) const;
        //! Get columns \c c1 through \c c2
        Mat<Num_T> get_cols(size_t c1, size_t c2) const;
        //! Get the columns specified by \c indexlist
        Mat<Num_T> get_cols(const Vec<size_t>& indexlist) const;
        //! Set row \c r to vector \c v
        void set_row(size_t r, const Vec<Num_T>& v);
        //! Set column \c c to vector \c v
        void set_col(size_t c, const Vec<Num_T>& v);
        //! Set rows to matrix \c m, staring from row \c r
        void set_rows(size_t r, const Mat<Num_T>& m);
        //! Set columns to matrix \c m, starting from column \c c
        void set_cols(size_t c, const Mat<Num_T>& m);
        //! Copy row \c from onto row \c to
        void copy_row(size_t to, size_t from);
        //! Copy column \c from onto column \c to
        void copy_col(size_t to, size_t from);
        ...
        Num_T& _elem(size_t r, size_t c) { return data[r + c * no_rows]; }
        //! Get element (r,c) from matrix without boundary check (not recommended to use)
        const Num_T& _elem(size_t r, size_t c) const { return data[r + c * no_rows]; }
        ...
        //! Access to the internal data structure (not recommended to use)
        size_t _datasize() const { return datasize; }
        //! Access to the internal data structure (not recommended to use)
    ...
    protected:
        ...
        //! Flag to indicate whether external storage is used
        bool isExternalStorage;

        /*! Protected integer variables
         * @{ */
        size_t datasize, no_rows, no_cols;
        /*! @} */
    ...
    private:
        ...
        bool in_range(size_t r, size_t c) const {
            return ((r >= 0) && (r < no_rows) && (c >= 0) && (c < no_cols));
        }
        //! Check whether row \c r is in the allowed range
        bool row_in_range(size_t r) const { return ((r >= 0) && (r < no_rows)); }
        //! Check whether column \c c is in the allowed range
        bool col_in_range(size_t c) const { return ((c >= 0) && (c < no_cols)); }
        //! Check whether element \c i is in the allowed range
        bool in_range(size_t i) const { return ((i >= 0) && (i < datasize)); }
    };
    ...
    */
    ...
    /*!
      \relates Mat
      \brief long long matrix
    */
    typedef Mat<long long> llmat;
...
namespace ZK
{
    ...
    */
    ...
    template<class Num_T> inline
        void Mat<Num_T>::alloc(size_t rows, size_t cols)
    {
        if ((rows > 0) && (cols > 0)) {
            datasize = rows * cols;
            no_rows = rows;
    ...
    template<class Num_T> inline
        ...
        datasize(0), no_rows(0), no_cols(0), data(0), factory(f), isExternalStorage(false) {
    }
    ...
    template<class Num_T> inline
        ...
        datasize(0), no_rows(0), no_cols(0), data(0), factory(f), isExternalStorage(false)
    {
        ZKIC_Assert((rows >= 0) && (cols >= 0), "Mat<>::Mat(): Wrong size");
        alloc(rows, cols);
    }

    template<class Num_T> inline
        Mat<Num_T>::Mat(const Mat<Num_T>& m) :
        datasize(0), no_rows(0), no_cols(0), data(0), factory(m.factory), isExternalStorage(false)
    {
    ...
    template<class Num_T>
    Mat<Num_T>::Mat(Num_T* const c_array, int rows, int cols, bool row_major,
        const Factory& f) :
        datasize(rows* cols), no_rows(rows), no_cols(cols), data(nullptr), factory(f), isExternalStorage(true)
    {
        Num_T* mutablePtr = const_cast<Num_T*>(c_array);
        if (row_major)
        {
            std::vector<Num_T>temp(datasize);

            for (int i = 0;i < rows;++i)
                for (int j = 0;j < cols;++j)
                    temp[i + j * no_rows] = mutablePtr[i * cols + j];

            std::memcpy(mutablePtr, temp.data(), sizeof(Num_T) * datasize);
        }
        data = mutablePtr;
    }
    ...
    void Mat<Num_T>::set_size(int rows, int cols, bool copy)
    {
        ...
        if (isExternalStorage)
        {
            ...
            {
                throw std::runtime_error("Mat<>::set_size(): Cannot resize mat with external storage to a larger size");
            }
        }
        else
        {
    ...
    {
        ZKIC_Assert(in_range(i), "Mat<>::operator(): Index out of range");
        return data[i];
    }
    ...
    {
        ZKIC_Assert(in_range(r, c), "Mat<>::set(): Indexing out of range");
        data[r + c * no_rows] = t;
    }
    ...
    template<class Num_T> inline
        Mat<Num_T> Mat<Num_T>::operator()(size_t r1, size_t r2, size_t c1, size_t c2) const
    {
    ...
    template<class Num_T> inline
        Mat<Num_T> Mat<Num_T>::get(size_t r1, size_t r2, size_t c1, size_t c2) const
    {
        return (*this)(r1, r2, c1, c2);
    }

    template<class Num_T> inline
        Vec<Num_T> Mat<Num_T>::get_row(size_t r) const
    {
        ZKIC_Assert(row_in_range(r), "Mat<>::get_row(): Index out of range");
        Vec<Num_T> a(no_cols);
    ...
    template<class Num_T>
    Mat<Num_T> Mat<Num_T>::get_rows(size_t r1, size_t r2) const
    {
        ZKIC_Assert((r1 >= 0) && (r1 <= r2) && (r2 < no_rows),
    ...
    template<class Num_T>
    Mat<Num_T> Mat<Num_T>::get_rows(const Vec<size_t>& indexlist) const
    {
        Mat<Num_T> m(indexlist.size(), no_cols);

        for (size_t i = 0;i < indexlist.size();i++) {
            ZKIC_Assert(row_in_range(indexlist(i)),
    ...
    template<class Num_T> inline
        Vec<Num_T> Mat<Num_T>::get_col(size_t c) const
    {
        ZKIC_Assert(col_in_range(c), "Mat<>::get_col(): Index out of range");
        Vec<Num_T> a(no_rows);
    ...
    template<class Num_T>
    Mat<Num_T> Mat<Num_T>::get_cols(size_t c1, size_t c2) const
    {
        ZKIC_Assert((c1 >= 0) && (c1 <= c2) && (c2 < no_cols),
    ...
    template<class Num_T>
    Mat<Num_T> Mat<Num_T>::get_cols(const Vec<size_t>& indexlist) const
    {
        Mat<Num_T> m(no_rows, indexlist.size());

        for (size_t i = 0; i < indexlist.size(); i++) {
            ZKIC_Assert(col_in_range(indexlist(i)),
    ...
    template<class Num_T> inline
        void Mat<Num_T>::set_row(size_t r, const Vec<Num_T>& v)
    {
        ZKIC_Assert(row_in_range(r), "Mat<>::set_row(): Index out of range");
        ZKIC_Assert(v.size() == no_cols,
            "Mat<>::set_row(): Wrong size of input vector");
        copy_vector(v.size(), v._data(), 1, data + r, no_rows);
    }

    template<class Num_T> inline
        void Mat<Num_T>::set_col(size_t c, const Vec<Num_T>& v)
    {
        ZKIC_Assert(col_in_range(c), "Mat<>::set_col(): Index out of range");
        ZKIC_Assert(v.size() == no_rows,
    ...
    template<class Num_T>
    void Mat<Num_T>::set_rows(size_t r, const Mat<Num_T>& m)
    {
        ZKIC_Assert(row_in_range(r), "Mat<>::set_rows(): Index out of range");
        ZKIC_Assert(no_cols == m.cols(),
            "Mat<>::set_rows(): Column sizes do not match");
        ZKIC_Assert(m.rows() + r <= no_rows,
    ...
    template<class Num_T>
    void Mat<Num_T>::set_cols(size_t c, const Mat<Num_T>& m)
    {
        ZKIC_Assert(col_in_range(c), "Mat<>::set_cols(): Index out of range");
        ZKIC_Assert(no_rows == m.rows(),
            "Mat<>::set_cols(): Row sizes do not match");
        ZKIC_Assert(m.cols() + c <= no_cols,
    ...
    template<class Num_T> inline
        void Mat<Num_T>::copy_row(size_t to, size_t from)
    {
        ZKIC_Assert(row_in_range(to) && row_in_range(from),
    ...
    template<class Num_T> inline
        void Mat<Num_T>::copy_col(size_t to, size_t from)
    {
        ZKIC_Assert(col_in_range(to) && col_in_range(from),
    ...
    template<class Num_T> inline
        void Mat<Num_T>::swap_rows(size_t r1, size_t r2)
    {
        ZKIC_Assert(row_in_range(r1) && row_in_range(r2),
    ...
    template<class Num_T> inline
        void Mat<Num_T>::swap_cols(size_t c1, size_t c2)
    {
        ZKIC_Assert(col_in_range(c1) && col_in_range(c2),
    ...
    template<class Num_T>
    void Mat<Num_T>::set_submatrix(size_t r1, size_t, size_t c1, size_t, const Mat<Num_T>& m)
    {
    ...
    template<class Num_T> inline
        void Mat<Num_T>::set_submatrix(size_t r, size_t c, const Mat<Num_T>& m)
    {
        ZKIC_Assert((r >= 0) && (r + m.no_rows <= no_rows) &&
            (c >= 0) && (c + m.no_cols <= no_cols),
            "Mat<>::set_submatrix(): Indexing out of range "
            "or wrong input matrix");
        for (size_t i = 0; i < m.no_cols; i++)
    ...
    template<class Num_T> inline
        void Mat<Num_T>::set_submatrix(size_t r1, size_t r2, size_t c1, size_t c2, Num_T t)
    {
        if (r1 == -1) r1 = no_rows - 1;
        if (r2 == -1) r2 = no_rows - 1;
        if (c1 == -1) c1 = no_cols - 1;
    ...
    template<class Num_T>
    void Mat<Num_T>::del_row(size_t r)
    {
        ZKIC_Assert(row_in_range(r), "Mat<>::del_row(): Index out of range");
        Mat<Num_T> Temp(*this);
        set_size(no_rows - 1, no_cols, false);
        for (size_t i = 0; i < r; i++) {
            copy_vector(no_cols, &Temp.data[i], no_rows + 1, &data[i], no_rows);
        }
        for (size_t i = r; i < no_rows; i++) {
    ...
    template<class Num_T>
    void Mat<Num_T>::del_col(size_t c)
    {
        ZKIC_Assert(col_in_range(c), "Mat<>::del_col(): Index out of range");
        Mat<Num_T> Temp(*this);
    ...
    template<class Num_T>
    void Mat<Num_T>::del_cols(size_t c1, size_t c2)
    {
        ZKIC_Assert((c1 >= 0) && (c1 <= c2) && (c2 < no_cols),
            "Mat<>::del_cols(): Indexing out of range");
        Mat<Num_T> Temp(*this);
        size_t n_deleted_cols = c2 - c1 + 1;
    ...
    template<class Num_T>
    void Mat<Num_T>::ins_row(size_t r, const Vec<Num_T>& v)
    {
        ZKIC_Assert((r >= 0) && (r <= no_rows),
            "Mat<>::ins_row(): Index out of range");
        ZKIC_Assert((v.size() == no_cols) || (no_rows == 0),
        ...
        for (size_t i = r + 1; i < no_rows; i++) {
            copy_vector(no_cols, &Temp.data[i - 1], no_rows - 1, &data[i], no_rows);
        }
    }

    template<class Num_T>
    void Mat<Num_T>::ins_col(size_t c, const Vec<Num_T>& v)
    {
        ZKIC_Assert((c >= 0) && (c <= no_cols),
            "Mat<>::ins_col(): Index out of range");
        ZKIC_Assert((v.size() == no_rows) || (no_cols == 0),
    ...
    //! \cond
    template<>
    ZKIC_DLL_EXPORT cmat cmat::hermitian_transpose() const;
    //! \endcond
    ...
    Mat<Num_T> concat_horizontal(const Mat<Num_T>& m1, const Mat<Num_T>& m2)
    {
        ...
        if (m2.no_cols == 0)
            return m1;
        ZKIC_Assert(m1.no_rows == m2.no_rows,
    ...
    Mat<Num_T> concat_vertical(const Mat<Num_T>& m1, const Mat<Num_T>& m2)
    {
        ...
        if (m2.no_rows == 0)
            return m1;
        ZKIC_Assert(m1.no_cols == m2.no_cols,
    ...
    {
        ZKIC_Assert(((no_rows == 1) && (no_cols == v.size()))
    ...
    Mat<Num_T>& Mat<Num_T>::operator+=(const Mat<Num_T>& m)
    {
        if (datasize == 0)
            ...
            ZKIC_Assert(m.no_rows == no_rows && m.no_cols == no_cols, "Mat<Num_T>::operator+=: wrong sizes");
            for (i = 0; i < no_cols; i++) {
                for (j = 0; j < no_rows; j++)
    ...
    Mat<Num_T> operator+(const Mat<Num_T>& m1, const Mat<Num_T>& m2)
    {
        ...
        ZKIC_Assert((m1.no_rows == m2.no_rows) && (m1.no_cols == m2.no_cols),
    ...
    Mat<Num_T>& Mat<Num_T>::operator-=(const Mat<Num_T>& m)
    {
        ...
        if (datasize == 0) {
            ...
            ZKIC_Assert((m.no_rows == no_rows) && (m.no_cols == no_cols),
                "Mat<>::operator-=(): Wrong sizes");
            for (i = 0; i < no_cols; i++) {
                for (j = 0; j < no_rows; j++)
                    data[pos + j] -= m.data[m_pos + j];
    ...
    Mat<Num_T>& Mat<Num_T>::operator*=(const Mat<Num_T>& m)
    {
        ZKIC_Assert(no_cols == m.no_rows, "Mat<>::operator*=(): Wrong sizes");
    ...
    //! \cond
    template<> ZKIC_DLL_EXPORT mat& mat::operator*=(const mat& m);
    template<> ZKIC_DLL_EXPORT cmat& cmat::operator*=(const cmat& m);
    //! \endcond
    ...
    //! \cond
    template<> ZKIC_DLL_EXPORT mat operator*(const mat& m1, const mat& m2);
    template<> ZKIC_DLL_EXPORT cmat operator*(const cmat& m1, const cmat& m2);
    //! \endcond
    ...
    //! \cond
    template<> ZKIC_DLL_EXPORT vec operator*(const mat& m, const vec& v);
    template<> ZKIC_DLL_EXPORT cvec operator*(const cmat& m, const cvec& v);
    //! \endcond
    ...
    void elem_mult_out(const Mat<Num_T>& m1, const Mat<Num_T>& m2,
        const Mat<Num_T>& m3, Mat<Num_T>& out)
    {
        ZKIC_Assert((m1.no_rows == m2.no_rows) && (m1.no_rows == m3.no_rows)
    ...
    ZKIC_DLL_EXPORT_TEMPLATE template class ZKIC_DLL_EXPORT Mat<double>;
    ZKIC_DLL_EXPORT_TEMPLATE template class ZKIC_DLL_EXPORT Mat<std::complex<double> >;
    ZKIC_DLL_EXPORT_TEMPLATE template class ZKIC_DLL_EXPORT Mat<int>;
    ZKIC_DLL_EXPORT_TEMPLATE template class ZKIC_DLL_EXPORT Mat<short int>;
    ZKIC_DLL_EXPORT_TEMPLATE template class ZKIC_DLL_EXPORT Mat<bin>;
    ZKIC_DLL_EXPORT_TEMPLATE template class ZKIC_DLL_EXPORT Mat<long long>;

    // addition operators

    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  mat operator+(const mat& m1, const mat& m2);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  cmat operator+(const cmat& m1, const cmat& m2);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  imat operator+(const imat& m1, const imat& m2);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  smat operator+(const smat& m1, const smat& m2);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  bmat operator+(const bmat& m1, const bmat& m2);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  llmat operator+(const llmat& m1, const llmat& m2);

    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  mat operator+(const mat& m, double t);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  cmat operator+(const cmat& m, std::complex<double> t);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  imat operator+(const imat& m, int t);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  smat operator+(const smat& m, short t);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  bmat operator+(const bmat& m, bin t);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  llmat operator+(const llmat& m, long long t);

    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  mat operator+(double t, const mat& m);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  cmat operator+(std::complex<double> t, const cmat& m);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  imat operator+(int t, const imat& m);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  smat operator+(short t, const smat& m);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  bmat operator+(bin t, const bmat& m);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  llmat operator+(long long t, const llmat& m);
    ...
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  mat operator-(const mat& m);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  cmat operator-(const cmat& m);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  imat operator-(const imat& m);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  smat operator-(const smat& m);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  bmat operator-(const bmat& m);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  llmat operator-(const llmat& m);
    ...
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  ivec operator*(const imat& m, const ivec& v);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  svec operator*(const smat& m, const svec& v);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  bvec operator*(const bmat& m, const bvec& v);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  llvec operator*(const llmat& m, const llvec& v);
    ...
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  mat elem_mult(const mat& m1, const mat& m2);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  cmat elem_mult(const cmat& m1, const cmat& m2);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  imat elem_mult(const imat& m1, const imat& m2);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  smat elem_mult(const smat& m1, const smat& m2);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  bmat elem_mult(const bmat& m1, const bmat& m2);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  llmat elem_mult(const llmat& m1, const llmat& m2);

    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  void elem_mult_out(const mat& m1, const mat& m2, mat& out);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  void elem_mult_out(const cmat& m1, const cmat& m2,
        cmat& out);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  void elem_mult_out(const imat& m1, const imat& m2,
        imat& out);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  void elem_mult_out(const smat& m1, const smat& m2,
        smat& out);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  void elem_mult_out(const bmat& m1, const bmat& m2,
        bmat& out);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  void elem_mult_out(const llmat& m1, const llmat& m2,
        llmat& out);

    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  void elem_mult_out(const mat& m1, const mat& m2,
    ...
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  void elem_mult_out(const bmat& m1, const bmat& m2,
        ...
        bmat& out);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  void elem_mult_out(const llmat& m1, const llmat& m2,
        const llmat& m3, const llmat& m4,
        llmat& out);

    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  void elem_mult_inplace(const mat& m1, mat& m2);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  void elem_mult_inplace(const cmat& m1, cmat& m2);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  void elem_mult_inplace(const imat& m1, imat& m2);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  void elem_mult_inplace(const smat& m1, smat& m2);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  void elem_mult_inplace(const bmat& m1, bmat& m2);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  void elem_mult_inplace(const llmat& m1, llmat& m2);

    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  double elem_mult_sum(const mat& m1, const mat& m2);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  std::complex<double> elem_mult_sum(const cmat& m1,
        const cmat& m2);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  int elem_mult_sum(const imat& m1, const imat& m2);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  short elem_mult_sum(const smat& m1, const smat& m2);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  bin elem_mult_sum(const bmat& m1, const bmat& m2);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  long long elem_mult_sum(const llmat& m1, const llmat& m2);
    ...
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  mat elem_div(const mat& m1, const mat& m2);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  cmat elem_div(const cmat& m1, const cmat& m2);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  imat elem_div(const imat& m1, const imat& m2);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  smat elem_div(const smat& m1, const smat& m2);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  bmat elem_div(const bmat& m1, const bmat& m2);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  llmat elem_div(const llmat& m1, const llmat& m2);

    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  void elem_div_out(const mat& m1, const mat& m2, mat& out);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  void elem_div_out(const cmat& m1, const cmat& m2, cmat& out);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  void elem_div_out(const imat& m1, const imat& m2, imat& out);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  void elem_div_out(const smat& m1, const smat& m2, smat& out);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  void elem_div_out(const bmat& m1, const bmat& m2, bmat& out);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  void elem_div_out(const llmat& m1, const llmat& m2, llmat& out);

    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  double elem_div_sum(const mat& m1, const mat& m2);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  std::complex<double> elem_div_sum(const cmat& m1,
        const cmat& m2);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  int elem_div_sum(const imat& m1, const imat& m2);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  short elem_div_sum(const smat& m1, const smat& m2);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  bin elem_div_sum(const bmat& m1, const bmat& m2);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  long long elem_div_sum(const llmat& m1, const llmat& m2);

    // concatenation

    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  mat concat_horizontal(const mat& m1, const mat& m2);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  cmat concat_horizontal(const cmat& m1, const cmat& m2);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  imat concat_horizontal(const imat& m1, const imat& m2);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  smat concat_horizontal(const smat& m1, const smat& m2);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  bmat concat_horizontal(const bmat& m1, const bmat& m2);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  llmat concat_horizontal(const llmat& m1, const llmat& m2);

    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  mat concat_vertical(const mat& m1, const mat& m2);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  cmat concat_vertical(const cmat& m1, const cmat& m2);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  imat concat_vertical(const imat& m1, const imat& m2);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  smat concat_vertical(const smat& m1, const smat& m2);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  bmat concat_vertical(const bmat& m1, const bmat& m2);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  llmat concat_vertical(const llmat& m1, const llmat& m2);

    // I/O streams

    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  std::ostream& operator<<(std::ostream& os, const mat& m);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  std::ostream& operator<<(std::ostream& os, const cmat& m);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  std::ostream& operator<<(std::ostream& os, const imat& m);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  std::ostream& operator<<(std::ostream& os, const smat& m);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  std::ostream& operator<<(std::ostream& os, const bmat& m);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  std::ostream& operator<<(std::ostream& os, const llmat& m);

    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  std::istream& operator>>(std::istream& is, mat& m);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  std::istream& operator>>(std::istream& is, cmat& m);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  std::istream& operator>>(std::istream& is, imat& m);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  std::istream& operator>>(std::istream& is, smat& m);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  std::istream& operator>>(std::istream& is, bmat& m);
    ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT  std::istream& operator>>(std::istream& is, llmat& m);
