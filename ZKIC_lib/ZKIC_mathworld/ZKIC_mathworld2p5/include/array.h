...
{
  ZKIC_Assert(size >= 0, "Array::set_size(): New size must not be negative");
  if (ndata == size)
    return;
  if (copy) {
...
{
  ZKIC_Assert(in_range(i), "Array::operator(): Improper index");
  return data[i];
}
...
{
  ZKIC_Assert(in_range(i1) && in_range(i2) && (i2 >= i1),
...
const Array<T> Array<T>::operator()(const Array<int> &indices) const
{
  ...
  for (int i = 0; i < a.size(); i++) {
    ZKIC_Assert(in_range(indices(i)),
...
{
  ZKIC_Assert(in_range(n), "Array::left(): Index out of range");
  Array<T> tmp(n);
  for (int i = 0; i < n; ++i)
    tmp.data[i] = data[i];
  return tmp;
...
{
  ZKIC_Assert(in_range(n), "Array::right(): Index out of range");
  Array<T> tmp(n);
  for (int i = 0; i < n; ++i)
...
{
  ZKIC_Assert((pos >= 0) && (n > 0) && (pos + n <= ndata), "Array::mid(): Indexing out of range");
  Array<T> tmp(n);
  for (int i = 0; i < n; ++i)
...
{
  ZKIC_Assert(ndata > 0, "Array::shift_right(x): Array empty!");
  T ret;
...
{
  ZKIC_Assert(a.ndata <= ndata, "Array::shift_right(): Shift Array too large");
  Array<T> out(a.ndata);
...
{
  ZKIC_Assert(a.ndata <= ndata, "Array::shift_left(): Shift Array too large");
  Array<T> out(a.ndata);
...
{
  ZKIC_Assert(in_range(i) && in_range(j),
...
void Array<T>::set_subarray(int i1, int i2, const Array<T> &a)
{
  ...
  ZKIC_Assert(in_range(i1) && in_range(i2),
                  "Array<T>::set_subarray(): Indices out of range.");
  ZKIC_Assert(i2 >= i1, "Array<T>::set_subarray(): i2 >= i1 necessary.");
  ZKIC_Assert(i2 - i1 + 1 == a.ndata, "Array<T>::set_subarray(): Wrong sizes.");
