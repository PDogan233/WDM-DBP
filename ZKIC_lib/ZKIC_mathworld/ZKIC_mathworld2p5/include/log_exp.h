...
{
  ZKIC_Assert(n >= 0, "int2bits(): Improper argument value");
...
{
  ZKIC_Assert(n > 0, "levels2bits(): Improper argument value");
  return int2bits(--n);
}
...
ZKIC_DLL_EXPORT double log_add(double log_a, double log_b);
...
ZKIC_DLL_EXPORT vec log2(const vec &x);
//! log-2 of the elements
ZKIC_DLL_EXPORT mat log2(const mat &x);
...
{
  return apply_function<double>(ZK::logb, b, x);
}
//! log-b of \c x
