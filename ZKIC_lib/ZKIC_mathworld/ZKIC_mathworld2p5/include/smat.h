...
private:
  void init();
  void alloc_empty();
  void alloc(size_t row_data_size = 200);
  void free();
...
template <class T>
void Sparse_Mat<T>::alloc(size_t row_data_init)
{
  if (n_cols == 0)
    col = 0;
  else
...
{
  ZKIC_Assert(r >= 0 && r<n_rows && c >= 0 && c < n_cols, "Incorrect input indexes given");
  return col[c](r);
}
...
void Sparse_Mat<T>::set_submatrix(int r1, int r2, int c1, int c2, const Mat<T>& m)
{
  ...
  ZKIC_Assert(r1 >= 0 && r2 >= 0 && r1 < n_rows && r2 < n_rows &&
                  c1 >= 0 && c2 >= 0 && c1 < n_cols && c2 < n_cols, "Sparse_Mat<Num_T>::set_submatrix(): index out of range");

  ZKIC_Assert(r2 >= r1 && c2 >= c1, "Sparse_Mat<Num_T>::set_submatrix: r2<r1 or c2<c1");
  ZKIC_Assert(m.rows() == r2 - r1 + 1 && m.cols() == c2 - c1 + 1, "Mat<Num_T>::set_submatrix(): sizes don't match");
...
{
  ZKIC_Assert(r >= 0 && r + m.rows() <= n_rows &&
...
{
  ZKIC_Assert(r1 <= r2 && r1 >= 0 && r1 < n_rows && c1 <= c2 && c1 >= 0 && c1 < n_cols,
...
{
  ZKIC_Assert(c1 <= c2 && c1 >= 0 && c1 < n_cols, "Sparse_Mat<T>::get_submatrix_cols()");
...
{
  ZKIC_Assert(c >= 0 && c < n_cols, "Sparse_Mat<T>::get_col()");
  v = col[c];
}
...
{
  ZKIC_Assert(c >= 0 && c < n_cols, "Sparse_Mat<T>::set_col()");
  col[c] = v;
}
...
void Sparse_Mat<T>::operator+=(const Sparse_Mat<T> &m)
{
  ZKIC_Assert(m.rows() == n_rows && m.cols() == n_cols, "Addition of unequal sized matrices is not allowed");
...
void Sparse_Mat<T>::operator-=(const Sparse_Mat<T> &m)
{
  ZKIC_Assert(m.rows() == n_rows && m.cols() == n_cols, "Subtraction of unequal sized matrices is not allowed");
...
Sparse_Mat<T> operator+(const Sparse_Mat<T> &m1, const Sparse_Mat<T> &m2)
{
  ZKIC_Assert(m1.n_cols == m2.n_cols && m1.n_rows == m2.n_rows , "Sparse_Mat<T> + Sparse_Mat<T>");
...
Sparse_Mat<T> operator*(const Sparse_Mat<T> &m1, const Sparse_Mat<T> &m2)
{
  ZKIC_Assert(m1.n_cols == m2.n_rows, "Sparse_Mat<T> * Sparse_Mat<T>");
...
Sparse_Vec<T> operator*(const Sparse_Mat<T> &m, const Sparse_Vec<T> &v)
{
  ZKIC_Assert(m.n_cols == v.size(), "Sparse_Mat<T> * Sparse_Vec<T>");
...
Vec<T> operator*(const Sparse_Mat<T> &m, const Vec<T> &v)
{
  ZKIC_Assert(m.n_cols == v.size(), "Sparse_Mat<T> * Vec<T>");
...
Vec<T> operator*(const Vec<T> &v, const Sparse_Mat<T> &m)
{
  ZKIC_Assert(v.size() == m.n_rows, "Vec<T> * Sparse_Mat<T>");
...
Sparse_Mat<T> trans_mult(const Sparse_Mat<T> &m1, const Sparse_Mat<T> &m2)
{
  ZKIC_Assert(m1.n_rows == m2.n_rows, "trans_mult()");
...
ZKIC_DLL_EXPORT_TEMPLATE template class ZKIC_DLL_EXPORT Sparse_Mat<int>;
ZKIC_DLL_EXPORT_TEMPLATE template class ZKIC_DLL_EXPORT Sparse_Mat<double>;
ZKIC_DLL_EXPORT_TEMPLATE template class ZKIC_DLL_EXPORT Sparse_Mat<std::complex<double> >;

ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT sparse_imat operator+(const sparse_imat &, const sparse_imat &);
ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT sparse_mat operator+(const sparse_mat &, const sparse_mat &);
ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT sparse_cmat operator+(const sparse_cmat &, const sparse_cmat &);

ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT sparse_imat operator*(const sparse_imat &, const sparse_imat &);
ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT sparse_mat operator*(const sparse_mat &, const sparse_mat &);
ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT sparse_cmat operator*(const sparse_cmat &, const sparse_cmat &);

ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT ivec operator*(const ivec &, const sparse_imat &);
ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT vec operator*(const vec &, const sparse_mat &);
ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT cvec operator*(const cvec &, const sparse_cmat &);

ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT ivec operator*(const sparse_imat &, const ivec &);
ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT vec operator*(const sparse_mat &, const vec &);
ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT cvec operator*(const sparse_cmat &, const cvec &);

ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT imat trans_mult(const sparse_imat &);
ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT mat trans_mult(const sparse_mat &);
ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT cmat trans_mult(const sparse_cmat &);

ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT sparse_imat trans_mult_s(const sparse_imat &);
ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT sparse_mat trans_mult_s(const sparse_mat &);
ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT sparse_cmat trans_mult_s(const sparse_cmat &);

ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT sparse_imat trans_mult(const sparse_imat &, const sparse_imat &);
ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT sparse_mat trans_mult(const sparse_mat &, const sparse_mat &);
ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT sparse_cmat trans_mult(const sparse_cmat &, const sparse_cmat &);

ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT ivec trans_mult(const sparse_imat &, const ivec &);
ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT vec trans_mult(const sparse_mat &, const vec &);
ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT cvec trans_mult(const sparse_cmat &, const cvec &);

ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT sparse_imat mult_trans(const sparse_imat &, const sparse_imat &);
ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT sparse_mat mult_trans(const sparse_mat &, const sparse_mat &);
ZKIC_DLL_EXPORT_TEMPLATE template ZKIC_DLL_EXPORT sparse_cmat mult_trans(const sparse_cmat &, const sparse_cmat &);
