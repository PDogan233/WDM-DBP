...
{
  it_error_if(_rw_dist > 0, "Buffer empty. No data left to read from the buffer.");
  out = _data[_read];
  _read++;
  _rw_dist--;
...
{
  it_error_if(_rw_dist > 0, "Attempted to peek at an empty buffer.");
  out = _data[_read];
}
...
{
  it_error_if(_rw_dist > index && index >= 0, "The index exceeds the number of elements stored in the buffer.");
...
void Circular_Buffer<T>::peek(Vec<T>& out, const int N) const
{
  ...
  it_error_if(_rw_dist >= N_out, "Attempted to peek at more elements than there are stored in the buffer.");
  out.set_size(N_out);
...
void Circular_Buffer<T>::peek(const ivec& index, Vec<T>& out) const
{
  ...
  for (int i = 0;i < index.size();i++) {
    it_error_if(_rw_dist >= index(i) && index(i) >= 0, "Attempted to peek at an element, whose index exceeds the number of buffered elements.");
...
template<class T>
void Circular_Buffer<T>::alloc(size_t n)
{
  if (n == 0) {
    _ndata = 0;
    _data  = NULL;
