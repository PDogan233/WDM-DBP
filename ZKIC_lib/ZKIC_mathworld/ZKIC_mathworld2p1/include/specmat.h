...
/*!
  ...
  \ingroup miscfunc
*/
ZKIC_DLL_EXPORT ivec find(const bvec &invector);
...
ZKIC_DLL_EXPORT vec ones(int size);
//! A Binary vector of ones
ZKIC_DLL_EXPORT bvec ones_b(int size);
//! A Int vector of ones
ZKIC_DLL_EXPORT ivec ones_i(int size);
//! A float Complex vector of ones
ZKIC_DLL_EXPORT cvec ones_c(int size);

//! A float (rows,cols)-matrix of ones
ZKIC_DLL_EXPORT mat ones(int rows, int cols);
//! A Binary (rows,cols)-matrix of ones
ZKIC_DLL_EXPORT bmat ones_b(int rows, int cols);
//! A Int (rows,cols)-matrix of ones
ZKIC_DLL_EXPORT imat ones_i(int rows, int cols);
//! A Double Complex (rows,cols)-matrix of ones
ZKIC_DLL_EXPORT cmat ones_c(int rows, int cols);

//! A Double vector of zeros
ZKIC_DLL_EXPORT vec zeros(int size);
//! A Binary vector of zeros
ZKIC_DLL_EXPORT bvec zeros_b(int size);
//! A Int vector of zeros
ZKIC_DLL_EXPORT ivec zeros_i(int size);
//! A Double Complex vector of zeros
ZKIC_DLL_EXPORT cvec zeros_c(int size);

//! A Double (rows,cols)-matrix of zeros
ZKIC_DLL_EXPORT mat zeros(int rows, int cols);
//! A Binary (rows,cols)-matrix of zeros
ZKIC_DLL_EXPORT bmat zeros_b(int rows, int cols);
//! A Int (rows,cols)-matrix of zeros
ZKIC_DLL_EXPORT imat zeros_i(int rows, int cols);
//! A Double Complex (rows,cols)-matrix of zeros
ZKIC_DLL_EXPORT cmat zeros_c(int rows, int cols);

//! A Double (size,size) unit matrix
ZKIC_DLL_EXPORT mat eye(int size);
//! A Binary (size,size) unit matrix
ZKIC_DLL_EXPORT bmat eye_b(int size);
//! A Int (size,size) unit matrix
ZKIC_DLL_EXPORT imat eye_i(int size);
//! A Double Complex (size,size) unit matrix
ZKIC_DLL_EXPORT cmat eye_c(int size);
...
//! Impulse vector
ZKIC_DLL_EXPORT vec impulse(int size);

//! linspace (works in the same way as the MATLAB version)
ZKIC_DLL_EXPORT vec linspace(double from, double to, int length = 100);
...
Vec<T> linspace_fixed_step(T from, T to, T step = 1)
{
    int points = 0;
    if (0 != step) {
        points = ZK::floor_i(double(to-from)/step)+1;
    }
    if (0 >= points) {
        return Vec<T>(0);
    }
...
*/
ZKIC_DLL_EXPORT vec zigzag_space(double t0, double t1, int K = 5);
 ...
 */
ZKIC_DLL_EXPORT imat hadamard(int size);
...
*/
ZKIC_DLL_EXPORT imat jacobsthal(int p);
...
/*!
  ...
  North-Holland, 1977.
*/
ZKIC_DLL_EXPORT imat conference(int n);
...
ZKIC_DLL_EXPORT const cmat toeplitz(const cvec &c);
...
/*!
  ...
  \ingroup miscfunc
*/
ZKIC_DLL_EXPORT mat rotation_matrix(int dim, int plane1, int plane2, double angle);

/*!
  \brief Calcualte the Householder vector
  \ingroup miscfunc
*/
ZKIC_DLL_EXPORT void house(const vec &x, vec &v, double &beta);

/*!
  \brief Calculate the Givens rotation values
  \ingroup miscfunc
*/
ZKIC_DLL_EXPORT void givens(double a, double b, double &c, double &s);

/*!
  \brief Calculate the Givens rotation matrix
  \ingroup miscfunc
*/
ZKIC_DLL_EXPORT void givens(double a, double b, mat &m);

/*!
  \brief Calculate the Givens rotation matrix
  \ingroup miscfunc
*/
ZKIC_DLL_EXPORT mat givens(double a, double b);

/*!
  \brief Calculate the transposed Givens rotation matrix
  \ingroup miscfunc
*/
ZKIC_DLL_EXPORT void givens_t(double a, double b, mat &m);

/*!
  \brief Calculate the transposed Givens rotation matrix
  \ingroup miscfunc
*/
ZKIC_DLL_EXPORT mat givens_t(double a, double b);
